...
public sealed partial class SwapToHandOperator : HTNOperator
{
    ...
    public override async Task<(bool Valid, Dictionary<string, object>? Effects)> Plan(NPCBlackboard blackboard, CancellationToken cancelToken)
    {
        ...
        return (true, new Dictionary<string, object>()
        {
            ...
            {
                NPCBlackboard.ActiveHandFree, _entityManager.System<HandsSystem>().HandIsEmpty((ownerUid, handsComponent), handId)
            },
        });
    }
    ...
    public override HTNOperatorStatus Update(NPCBlackboard blackboard, float frameTime)
    {
        ...
        if (!_entityManager.System<HandsSystem>().TrySetActiveHand(ownerUid, blackboard.GetValue<string>(Key)))
